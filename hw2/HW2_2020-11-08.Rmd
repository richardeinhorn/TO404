...
The above graph allows us to understand bikesharing seasonality. Most trips occur between April and October, with the summer months (blue) generally most attractive to Citibike cyclists. This seasonality is most likely explained by weather and tourist influx. There is an interesting spike in trips in February - further analysis would need to be done to see what boosted bike usage. Potentially effects include a warm weather episode or an increase in tourists due to the holidays around February.
...
g <- ggplot(weekday_datas, aes(x = weekday, y = n, fill = type)) +
